...
module.exports = {
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                if (req.body.type === 'Docente') {
                    ...
                    await person.update({student_state: true}, {transaction: t});
                }
                ...
                if (user) {
                    ...
                    let out = await abox.templateSendUserCredential(
                        person.name + ' ' + person.paternal + ' ' + person.maternal,
                        person.document_number, passWord);
                    const mailOptions = {
                    ...
                    if (sendMail) {
                        await user.update({pass: hashCode}, {transaction: t});
                    } else {
    ...
    validUserIntranet: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                if (!validPass) throw {message: "Contraseña o usuario incorrecto"};
                let token = jwt.sign({id: valid.id}, "mysecretkey", {expiresIn: 10800});
                data = {
                    message: 'Access',
                    info: true,
                    token: token,
            ...
            // res.redirect('http://localhost:3000/auth/valid/' + req.user);
    ...
    retriveDataAdministrativeIntranet: async (req, res) => {
        try {
            const principalOrganicUnit = await System_configuration.findOne({
                attributes: ['id', 'description', 'description_document', 'abbreviation'],
                where: {state: true}
            });
    ...
    listStudentProgramIntranet: async (req, res) => {
        try {
            ...
            const data = await Student.findAll({
                ...
                include: {
                    attributes: ['id', 'id_academic_degree', 'denomination', 'description'],
                    model: Program,
                    as: 'Program'
                }
            })
                    ...
                    _data.push(
                        {
                            ...
                            "amount": r.amount,
                            "process": r.Academic_semester.Academic_calendar.denomination.substr(-4) + ' - ' + r.Academic_semester.denomination.substr(-1),
                            "type": r.type
                        }
                    )
                }
    ...
    createMovementIntranet: async (req, res) => {
        ...
        try {
            ...
            let movement = await Movement.findOne({where: {voucher_code: nameFile}});
            if (movement) {
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                let maxMovementID = await Movement.max('id', {paranoid: false}, {transaction: t});
        ...
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                array.push({
                    ...
                    'process': data[i].Schedule.Academic_semester.Academic_calendar.denomination.substr(-4) + '-' + data[i].Schedule.Academic_semester.denomination.substr(-2)
                })
            }
    ...
    listTeacherScheduleAdmissionPlan: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                    ...
                    schedule = await Schedule.findOne({where: {id: req.params.id_schedule}}, {transaction: t});
    ...
    listTeacherActaBookBySchedule: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                let temp = corre ? corre.Academic_semester.Academic_calendar.denomination.substr(-4) + '' + corre.Academic_semester.denomination.substr(-1) + '' + corre.id_course + ''
                    + corre.id_program + '' + corre.id_admission_plan + '' + corre.type + '-' + corre.correlative : '';
                code_acta = temp;
    ...
    reportTeacherActa: async (req, res) => {
        ...
        try {
            ...
            let correlative = acta ? acta.Academic_semester.Academic_calendar.denomination.substr(-4) + '' + acta.Academic_semester.denomination.substr(-1) + '' + acta.id_course + ''
                + acta.id_program + '' + acta.id_admission_plan + '' + acta.type + '-' + acta.correlative : '';
            ...
            const data = {
                ...
                semester: acta.Academic_semester.Academic_calendar.denomination.substr(-4) + '-' + acta.Academic_semester.denomination.substr(-1),
                Course: {
    ...
    listConceptIntranet: async (req, res) => {
        try {
            ...
            let totalCiclos = program && program.Plan && program.Plan.Ciclos.length

            let id = [];
            let type = '';
            if (program.Program.id_academic_degree == 2 || program.Program.id_academic_degree == 3) {
                type = 'Maestría'
            ...
            for (let i = 0; i < records.length; i++) {
                total = Math.round(parseFloat(records[i].percent) * parseFloat(actualUit.amount) / 100)
                if ((records[i].id == 59) || (records[i].id == 102) || (records[i].id == 120)) {
    ...
    listConceptOperation: async (req, res) => {
        try {
            ...
            if (program.Program.id_academic_degree == 2 || program.Program.id_academic_degree == 3) {
                ...
                type = 'Especialista'
                id = [3, 115, 102,113, 49, 39, 86, 98, 6, 122, 108, 107, 106,17,110]
            }
    ...
    listStudentConceptValidIntranet: async (req, res) => {
        ...
        try {
            ...
            requeriment = await Requeriment_delivered.findAll({                // attributes: ['id', 'type'],
                where: {
            ...
            const dataOne = await Payment.findAll({                // attributes: ['id', 'type'],
                where: {
                    id_student: req.params.id_student,
            ...
            const dataTwo = await Payment.findAll({                // attributes: ['id', 'type'],
                where: {
                    id_student: req.params.id_student,
                    denomination: {[Op.or]: ['Inscripción', 'Matrícula', 'Pensión']}
                },
                include: [
                    {
                        model: Concept,
            ...
            res.status(200).send({payments, pendientPayment, pendientRequeriment})
        } catch (err) {
            res.status(445).send({message: message.ERROR_TRANSACTION, error: err})
        }
    },
        ...
        try {
            await ST.transaction(async (t) => {
                ...
                for (let i = 0; i < temp.length; i++) {
                    ...
                    let maxPaymnetID = await Payment.max('id', {paranoid: false}, {transaction: t});
                    //listamos los conceptos de acuerdo a lo que selecciona el estudiante
                    let concept = await Concept.findByPk(temp[i]);
                    let amountActual = Math.round(parseFloat(concept.percent) * parseFloat(actualUit.amount) / 100)
    ...
    listProcedureStudentIntranet: async (req, res) => {
        ...
        try {
            ...
            let records = await Payment.findAll({
                attributes: ['id', 'amount', 'denomination', 'type', 'generate', 'state', 'created_at'],
                where: {
    ...
    createComprobanteProcedureIntranet: async (req, res) => {
        ...
        try {
            ...
            let movement = await Movement.findOne({where: {voucher_code: nameFile2}});
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                let movement = await Movement.findOne({where: {voucher_code: nameFile2}});
                if (movement) throw 'Ya se ha registrado el codigo de comprobante !!'
                let maxMovementID = await Movement.max('id', {paranoid: false}, {transaction: t});
                await Movement.create({
                    id: maxMovementID + 1,
                    id_student: req.body.id_student,
                    id_program: programData.Program.id,
                    id_organic_unit: programData.Program.id_unit_organic_register,
                    voucher_code: nameFile2,
                    voucher_url: archive,
                    type: 'Deposíto',
                    state: 'Registrado'
