 ...
 listRegistrationCourse: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                    ...
                    ciclo = await Schedule.findAll({
                        where: {id_process: req.body.id_process, type_registration: req.body.type_registration},
                        include: [
                            {
                                ...
                                include: [
                                    ...
                                    {
                                        attributes: ['ambient', 'days', 'end_time', 'start_time'],
                                        model: Horary,
                                        as: 'Horarys'
                                    },
                                ]
                    ...
                    approvedData = await Model.findAll({
                        ...
                        include: {
                            attributes: ['id', 'id_course', 'note', 'state'],
                            //note>=14 &&
                            where: {note: {[Op.gte]: 14}, state: "Aprobado"},
                            model: Registration_course,
                            as: "Registration_course",
                        }
                    }, {transaction: true});
                    //convertirmos la data en un solo array (approvedCourse)
                    approvedData.map(r => r.Registration_course.map(j => approvedCourse.push({
                        "id": j.id,
                        "id_course": j.id_course,
                        "note": j.note,
                        "state": j.state
                        ...
                        for (let i = 0; i < approvedCourse.length; i++) {
                            for (let j = 0; j < restCourse.length; j++) {
                                ...
                                if (restCourse[j].requirements) {
                                    for (let k = 0; k < restCourse[j].requirements.length; k++) {
                                        // preguntamos si el curso apronvado esta dentro de los requerimientos
                                        // y solo cargamos esos dichos cursos
                                        if (restCourse[j].requirements[k].value === approvedCourse[i].id_course) {
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                    registrationData = await Model.findAll({
                        attributes: ['id', 'id_semester', 'type', 'state', 'created_at', 'deleted_at'],
                        ...
                        include: [
                            ...
                            {
                                required: true,
                                attributes: ['id', 'id_course', 'id_schedule', 'note', 'type_course', 'state'],
                                model: Registration_course,
                                as: "Registration_course",
                                include: {
                    ...
                    for (let i = 0; i < registrationData.length; i++) {
                        registration.push({
                            ...
                            "Payment": {
                                ...
                                "Concept": {
                                    ...
                                    "denomination": registrationData[i].Payment.Concept.denomination,
                                }
                            },
                            "Registration_course": []
                        })
                        for (let j = 0; j < registrationData[i].Registration_course.length; j++) {
                            registration[i].Registration_course.push({
                                ...
                                "type_registration_course": registrationData[i].Registration_course[j].type_course,
                                ...
                                "state_registration_course": registrationData[i].Registration_course[j].state,
            ...
            await ST.transaction(async (t) => {
                    ...
                    for (let i = 0; i < registrationData.length; i++) {
                        registration.push({
                            ...
                            "type": registrationData[i].type,
                            "state": registrationData[i].state,
                            "id_academic_semester": registrationData[i].Academic_semester.id,
                            "academic_semester_denomination": registrationData[i].Academic_semester.denomination.substr(-1),
                            "academic_calendar_denomination": registrationData[i].Academic_semester.Academic_calendar.denomination.substr(-4),
                            "id_payment": registrationData[i].Payment.id,
                            "payment_concept_denomination": registrationData[i].Payment.Concept.denomination,
                            "id_payment_concept": registrationData[i].Payment.Concept.id,
                            "id_student_document": registrationData[i].Student_document.id,
                            "id_document": registrationData[i].Student_document.Document.id,
                            "document_archive": registrationData[i].Student_document.Document.archive,
                            "document_topic": registrationData[i].Student_document.Document.topic,
                        })
                    }
                }
            );
                ...
                include: [
                    {
                        ...
                        include: [
                            {
                                attributes: ['id', 'document_number', 'email', [Fn('CONCAT', Col('paternal'), ' ', Col('maternal'), ' ', Col('name')), 'name']],
                                model: Person,
                                as: "Person"
                            },
                            {
            ...
            await ST.transaction(async (t) => {
                    registrationData = await Model.findAll({
                        ...
                        include: [
                            ...
                            },
                            {
                                ...
                                as: "Registration_course",
                                include: {
                                    attributes: ['id', 'id_ciclo', 'denomination', 'order', 'credits', 'state'],
                                    model: Course,
                                    as: "Course",
                                    include: {
                                        attributes: ['ciclo'],
                    ...
                    for (let i = 0; i < registrationData.length; i++) {
                        registration.push({
                            ...
                            "Academic_semester": {
                                "id": registrationData[i].Academic_semester.id,
                                "denomination": registrationData[i].Academic_semester.denomination,
                                "Academic_calendar": {
                                    "id": registrationData[i].Academic_semester.Academic_calendar.id,
                                    "denomination": registrationData[i].Academic_semester.Academic_calendar.denomination.substr(-4),
                                }
    ...
    createRegistration: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                    ...
                    if (req.body.type === 'Regular' || req.body.type === 'Extemporánea') {
                    ...
                    let maxRegistrationID = await Model.max('id', {paranoid: false}, {transaction: t});
                    ...
                    if (!registration) {
                        ...
                        let maxPaymentID = await Payment.max('id', {paranoid: false}, {transaction: t});
                        ...
                        discounts = await Student_discount.findAll({
                            where: {id_student: req.body.id_student},
                            include: {attributes: ['id', 'amount'], model: Discount, as: 'Discount'}
                        }, {transaction: t});
                        ...
                        let pensionCostAmount = totalDiscount > 0 ? pensionCost.amount - pensionCost.amount * totalDiscount / 100 : pensionCost.amount;
                        let totalPension = pensionCost.cant / matriculaCost.cant;
                        let tempPension = [];
                        if (totalPension < 1) throw "No ha registrado correctamente los costos de admisión";


                        // BUSCAMOS EL NUMERO DE ORDEN DE ACUERDO A LA ULTIMA PENSION SEGUN EL ID Y EL MAYOR NUMERO
                        let endNumberPension = await Payment.max('order_number', {
                            where: {id_student: req.body.id_student, id_concept: pensionCost.id_concept}
                        }, {transaction: t});
                        ...
                        let totalDayClass = endDateClass.diff(startDateClass, 'days');
                        let incrementDay = Math.round(totalDayClass / totalPension);


                        //REGISTRAMOS LOS CONCEPTOS DE PENSION
                        let tempIncrementDay = moment(startDateClass).add(incrementDay, 'days');
                        if (req.body.type === 'Regular' || req.body.type === 'Extemporánea') {
                            for (let i = 1; i <= totalPension; i++) {
                                ...
                                let pensionCreate = await Payment.create({
                                    ...
                                    payment_date: moment(tempIncrementDay).format('YYYY-MM-DD'),
                                    order_number: _orderNumberPension + i,
                                    denomination: "Pensión",
                                    amount: pensionCostAmount,
                                }, {transaction: t});
                                tempPension.push(pensionCreate);
                                tempIncrementDay = moment(tempIncrementDay).add(incrementDay, 'days');
                            }
                        ...
                        for (let i = 0; i < dataCourse.length; i++) {
                            let maxRegistrationCourseID = await Registration_course.max('id', {paranoid: false}, {transaction: t});


                            let state = dataCourse[i].note >= 14 ? "Aprobado" : "Desaprobado";
                            let typeCourse = (req.body.type === 'Regular' || req.body.type === 'Extemporánea' || req.body.type === 'Curso desaprobado por Credito') ? 'R' :
                                (req.body.type === 'Curso dirigido Tesis' || req.body.type === 'Curso dirigido Otros') ? 'D' : 'A';


                            let registrationCourseCreate = await Registration_course.create({
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                paymentDataPension = await Payment.findAll({
                    where: {id_registration: req.params.id, denomination: "Pensión"}
                }, {transaction: t});
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                for (let i = 0; i < acta.length; i++) {
                    ...
                    if (acta[i].Schedule) {
                        if (acta[i].Schedule.Acta_book) throw {message: "No se puede Eliminar, Ya se genero acta del curso " + acta[i].id};
                    }


                }
                await Model.destroy({where: {id: req.params.id}}, {transaction: t});
                await Registration_course.destroy({where: {id_registration: req.params.id}}, {transaction: t});
    ...
    leaveRegistration: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                registrationData = await Model.findOne({where: {id: req.params.id}}, {transaction: t});
                registrationCourseData = await Registration_course.findAll({where: {id_registration: req.params.id}}, {transaction: t});
                // payment = await Payment.findOne({where: {id_registration: req.params.id}}, {transaction: t});

                await registrationData.update({state: "Retirado"}, {transaction: t});

                // await payment.update({type: "Retirado"}, {transaction: t});

                for (let i = 0; i < registrationCourseData.length; i++) {
                    let temp = await registrationCourseData[i].update({state: "Retirado"}, {transaction: t});
    ...
    createNoRegistration: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                await studentData.update({type: req.body.type}, {transaction: t});
                await Model.create({
                    id: maxRegistrationID + 1,
    ...
    updateStateRegistration: async (req, res) => {
        ...
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                let registrationCurses = JSON.parse(req.body.registrationCurses);
                let tempData = []
                for (let i = 0; i < registrationCurses.length; i++) {
                    let data = await Registration_course.findByPk(registrationCurses[i].id)
            ...
            await ST.transaction(async (t) => {
                ...
                if (records.length > 0) {
                    for (let i = 0; i < records.length; i++) {
                        tempCourse = tempCourse.concat(records[i].Registration_course)
                    }
            ...
            await ST.transaction(async (t) => {
                records = await Registration_course.findAll({
                    ...
                    include: {
                        ...
                        where: {
                            id_semester: req.params.id_academic_semester,
                            state: {[Op.ne]: 'Pendiente', [Op.ne]: 'No Matrículado'},
                        },
                        ...
                        include: {
                            ...
                            as: "Student",
                            include: {
                                attributes: ['id', 'document_number', 'email', 'photo', [Fn('CONCAT', Col('paternal'), ' ', Col('maternal'), ' ', Col('name')), 'name']],
                                model: Person,
                                as: "Person"
                            }
                        }
                    ...
                    order: [
                        [{model: Model, as: 'Registration'}, {model: Student, as: 'Student'}, {
                            model: Person,
                            as: 'Person'
                        }, 'paternal', 'asc']
                    ]
    ...
    updateRegistrationCourse: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                let dataCourse = JSON.parse(req.body.courses);
                for (let i = 0; i < dataCourse.length; i++) {
                    course = await Registration_course.findByPk(dataCourse[i].id)
                    let state = dataCourse[i].note >= 14 ? "Aprobado" : "Desaprobado";
                    registrationCourseCreate = await course.update({
                        note: dataCourse[i].note,
                        state: state,
                ...
                include: [
                    {
                        ...
                        as: "AS",
                        include: [
                            {attributes: ['id', 'denomination'], model: Academic_calendar, as: "AC"},
                            {attributes: ['id', 'denomination'], model: Semester, as: "Semester"}
                        ]
                    ...
                    },
                    {
                        ...
                        as: "Registration",
                        include: {
                            attributes: ['id', 'denomination', 'order', 'credits', 'type'],
                            model: Course,
                            as: "Course",
                            include: {
    ...
    updateRegistrationRetirement: async (req, res) => {
        try {
            ...
            let updatePayment;
            await ST.transaction(async (t) => {
                    registration = await Model.findOne({where: {id: req.body.id_registration}}, {transaction: t});
                    if (!registration) throw "Matricula no encontrada";
                    updateRegistration = await registration.update({type: registration.type + "-Retirado"}, {transaction: t});
                    ...
                    updatePayment = await payment.update({state: true}, {transaction: t});
                    //
                    // return registration;
                }
            );
