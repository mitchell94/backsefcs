    ...
    createActaBook: async (req, res) => {
        try {
            ...
            await ST.transaction(async (t) => {
                ...
                let semester = codeAcademicSemester.denomination.substr(-2) == ' I' ? '1' : '2';

                let digi = codeSchedule.group_class + codeAcademicSemester.Academic_calendar.denomination.substr(-2) + semester + codeCourse.code + req.body.type;

                //validadmos si existe
                let existActa = await Model.findOne({where: {correlative: digi}});
                if (existActa) {
                    throw {message: "Ya se ha generado un acta con este codigo"};
                }
            ...
            await ST.transaction(async (t) => {
                ...
                data = await Model.findAll({
                    ...
                    include: [
                        ...
                        {
                            attributes: ['group_class', 'start_date', 'start_date'],
                        ...
                        {
                            attributes: ['description'],
                            model: Admission_plan, as: 'Admission_plan'
                        },
                        {
                            attributes: ['id'],
                            model: Teacher,
