...
module.exports = {
    ...
    try {
      ...
      let result = await ST.transaction(async (t) => {
        ...
        if (student) {
          ...
          if (students && cost) {
            let concepts = JSON.parse(crypt.decrypt(req.body.concepts, k, v));
            ...
            for (let i = 0; i < concepts.length; i++) {
              ...
              if (concepts[i].Concepts_parent.denomination === "INSCRIPCIÓN") {
              ...
              payments = await Payment.create(
                {
                  ...
                  denomination: concepts[i].Concepts_parent.denomination,
              ...
              for (let h = 0; h < promiseFinal.length; h++) {
                if (promiseFinal[h].state) {
                  if (promiseFinal[h].state === true) {
                    if (promiseFinal[h].denomination === "INSCRIPCIÓN") {
                  ...
                  paymentDetails = await Payment_detail.create(
                    {
                      id: maxDetail + h + 1,
                      ...
                      order_number: promiseFinal[h].order_number,
                      type: "Pendiente",
  ...
  destroy: function (req, res) {
    return Model.findOne({
      ...
      .then((record) => {
        if (!record)
          return res.status(400).send({ message: message.RECORD_NOT_FOUND });
        return record
          .destroy()
    ...
    try {
      await ST.transaction(async (t) => {
        ...
        paymentData = await Payment.findOne({
          where: { id_student: req.params.id, denomination: "Inscripción" },
        });
  ...
  async destroyStudent(req, res) {
    try {
      await ST.transaction(async (t) => {
        ...
        const tempRequirementDeliverd = await Requeriment_delivered.findAll(
        ...
        for (let i = 0; i < tempStudentDiscount.length; i++) {
          let temp = await tempStudentDiscount[i].destroy({ transaction: t });
          array.push(temp);
        }
        ...
        for (let i = 0; i < tempMovement.length; i++) {
          let temp = await tempMovement[i].destroy({ transaction: t });
          array.push(temp);
        }
        for (let i = 0; i < tempRegistration.length; i++) {
          let temp = await tempRegistration[i].destroy({ transaction: t });
          let tempRegistrationCourse = await Registration_course.findAll(
            { where: { id_registration: tempRegistration[i].id } },
            { transaction: t }
          );
